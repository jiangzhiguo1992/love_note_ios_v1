...
class ExtensionTextView {
    
    public static func setTextViewTextWithPlaceholder(_ tv: UITextView?, text: String?) {
        tv?.text = text
        tv?.textChange(Notification(name: Notification.Name(rawValue: "")))
    }
...
extension UITextView {
    ...
    var placeholdFont: UIFont? {
        ...
        get {
            return objc_getAssociatedObject(self, UITextView.RuntimeKey.placeholdFont!) as? UIFont == nil ? UIFont.systemFont(ofSize: ViewHelper.FONT_SIZE_NORMAL) : objc_getAssociatedObject(self, UITextView.RuntimeKey.placeholdFont!) as? UIFont
        }
    }
    ...
    var limitLabelFont: UIFont? {
        ...
        get {
            return objc_getAssociatedObject(self, UITextView.RuntimeKey.limitLabelFont!) as? UIFont == nil ? UIFont.systemFont(ofSize: ViewHelper.FONT_SIZE_NORMAL) : objc_getAssociatedObject(self, UITextView.RuntimeKey.limitLabelFont!) as? UIFont
        }
    }
    ...
    fileprivate func initPlaceholder(_ placeholder: String) {
        ...
        let rect = placeholder.boundingRect(with: CGSize(width: self.frame.size.width - self.textContainer.lineFragmentPadding * 2, height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: self.placeholdFont!], context: nil)
        placeholderLabel?.frame = CGRect(x: self.textContainer.lineFragmentPadding, y: self.textContainerInset.top, width: rect.size.width, height: rect.size.height)
    ...
    fileprivate func initWordCountLabel(_ limitLength: NSNumber) {
        ...
        // let height = ViewUtils.getFontHeight(font: wordCountLabel?.font, width: frame.width, text: wordCountLabel?.text)
        wordCountLabel = UILabel(frame: CGRect(x: 0, y: self.frame.size.height - ScreenUtils.heightFit(20), width: self.frame.size.width, height: ScreenUtils.heightFit(20)))
    ...
    @objc fileprivate func textChange(_ notification: Notification) {
        ...
        if autoHeight == true && self.oldFrame != nil {
            ...
            UIView.animate(withDuration: 0.15) {
                self.frame = CGRect.init(x: (self.oldFrame?.origin.x)!, y: (self.oldFrame?.origin.y)!, width: (self.oldFrame?.size.width)!, height: size.height + ScreenUtils.heightFit(25) <= (self.oldFrame?.size.height)! ? (self.oldFrame?.size.height)! : size.height + ScreenUtils.heightFit(25))
            }
        }
    ...
    @objc fileprivate func getStringPlaceSize(_ string: String, textFont: UIFont) -> CGSize {
        ...
        let size = string.boundingRect(with: CGSize(width: self.contentSize.width - ScreenUtils.widthFit(10), height: CGFloat.greatestFiniteMagnitude), options: options, attributes: attribute, context: nil).size
        return size
    }
    ...
    override open func layoutSubviews() {
        ...
        if limitLength != nil && wordCountLabel != nil {
             ...
             */
            let height = ViewUtils.getFontHeight(font: wordCountLabel?.font, width: frame.width, text: wordCountLabel?.text)
            wordCountLabel!.frame = CGRect(x: 0, y: frame.height - height + contentOffset.y, width: frame.width, height: height)
        }
        if placeholder != nil && placeholderLabel != nil {
            let rect: CGRect = placeholder!.boundingRect(with: CGSize(width: frame.width - self.textContainer.lineFragmentPadding * 2, height: CGFloat.greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: self.font as Any], context: nil)
            placeholderLabel!.frame = CGRect(x: self.textContainer.lineFragmentPadding, y: self.textContainerInset.top, width: rect.size.width, height: rect.size.height)
        }
        if autoHeight == true {
