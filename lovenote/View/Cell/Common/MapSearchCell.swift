...
class MapSearchCell: BaseTableCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        // address
        lAddress = ViewHelper.getLabelBlackNormal(width: MapSearchCell.MAX_WIDTH, text: "-", lines: 0, align: .left)
        lAddress.frame.origin = CGPoint(x: MapSearchCell.MARGIN, y: MapSearchCell.MARGIN)
        
        // location
        lLocation = ViewHelper.getLabelGreySmall(width: MapSearchCell.MAX_WIDTH, text: "-", lines: 1, align: .left, mode: .byTruncatingMiddle)
        lLocation.frame.origin = CGPoint(x: MapSearchCell.MARGIN, y: lAddress.frame.origin.y + lAddress.frame.size.height + MapSearchCell.MARGIN)
    ...
    private static func getHeightByData(poi: AMapPOI) -> CGFloat {
        ...
        if totalHeight == nil || addressHeight == nil {
            let cell = MapSearchCell(style: .default, reuseIdentifier: MapSearchCell.ID)
    ...
    public static func getCellWithData(view: UITableView, indexPath: IndexPath, dataList: [AMapPOI]?) -> MapSearchCell {
        var cell = view.dequeueReusableCell(withIdentifier: ID, for: indexPath) as? MapSearchCell
        if cell == nil {
            cell = MapSearchCell(style: .default, reuseIdentifier: ID)
        }
