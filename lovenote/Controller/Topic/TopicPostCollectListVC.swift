...
class TopicPostCollectListVC: BaseVC, IndicatorInfoProvider, UITableViewDelegate, UITableViewDataSource {
    ...
    public static func get(height: CGFloat, mine: Bool) -> TopicPostCollectListVC {
        let vc = TopicPostCollectListVC(nibName: nil, bundle: nil)
        vc.height = height
        vc.mine = mine
        vc.itemInfo.title = mine ? StringUtils.getString("my_collect") : StringUtils.getString("ta_collect")
        return vc
    }
    ...
    private func refreshData(more: Bool) {
        page = more ? page + 1 : 0
        // api
        let api = Api.request(.topicPostCollectListGet(me: mine, page: page),
                              success: { (_, _, data) in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if postList == nil || postList!.count <= indexPath.row {
            return
        }
        let post = postList![indexPath.row]
        if mine && (post.isDelete() || post.screen) {
    ...
    @objc func showItemDeleteAlert(indexPath: IndexPath) {
        _ = AlertHelper.showAlert(title: StringUtils.getString("confirm_del_this_collect"),
                                  msg: nil,
                                  confirms: [StringUtils.getString("confirm_no_wrong")],
                                  cancel: StringUtils.getString("i_think_again"),
                                  canCancel: true,
                                  actionHandler: { (_, _, _) in
                                    self.delItemCollect(index: indexPath.row)
        },
                                  cancelHandler: nil)
    }
    
    private func delItemCollect(index: Int) {
        if postList == nil || postList!.count <= index {
            return
        }
        ...
        let api = Api.request(.topicPostCollectToggle(postCollect: body.toJSON()),
                              loading: true, cancel: true, success: { (_, _, data) in
                                ...
                                self.refreshHeightMap(refresh: true, start: 0, dataList: self.postList)
