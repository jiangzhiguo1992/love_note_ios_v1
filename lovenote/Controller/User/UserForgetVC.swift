...
class UserForgetVC: BaseVC {
    ...
    public static func pushVC() {
        AppDelegate.runOnMainAsync {
            RootVC.get().pushNext(UserForgetVC(nibName: nil, bundle: nil))
        }
    }
    
    override func initView() {
        // navigationBar
        initNavBar(title: "forget_pwd")
        ...
        // pwd-tf
        tfPwd = ViewHelper.getTextField(width: tfWidth, paddingV: padding, placeholder: StringUtils.getString("input_hint_new_pwd"), pwd: true)
        ...
        // pwd2-tf
        tfPwd2 = ViewHelper.getTextField(width: tfWidth, paddingV: padding, placeholder: StringUtils.getString("input_hint_new_pwd_confirm"), pwd: true)
        ...
        // forget-btn
        let btnForget = ViewHelper.getBtnBGPrimary(width: maxWidth / 2, paddingV: padding / 2, title: StringUtils.getString("set_ok"), circle: true)
        btnForget.center.x = self.view.center.x
        btnForget.frame.origin.y = vVerify.frame.origin.y + vVerify.frame.size.height + margin
        ...
        btnForget.addTarget(self, action: #selector(targetForget), for: .touchUpInside)
    }
    ...
    @objc private func targetSendCode(sender: UIButton) {
        ...
        let body = ApiHelper.getSmsForgetBody(phone: phone)
    ...
    @objc private func targetForget(sender: UIButton) {
        ...
        let api = Api.request(.userModify(type: ApiHelper.MODIFY_FORGET, code: code, oldPwd: "", user: body.toJSON()),
                              loading: true, success: { code, msg, data in
                                // 停止倒计时
                                self.stopCodeCountDown()
