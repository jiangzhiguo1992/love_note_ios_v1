...
class MoreMatchWifeOurVC: BaseVC, IndicatorInfoProvider, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public static func get(height: CGFloat) -> MoreMatchWifeOurVC {
        let vc = MoreMatchWifeOurVC(nibName: nil, bundle: nil)
        vc.height = height
        return vc
    }
    ...
    override func initView() {
        ...
        collectView = ViewUtils.getCollectionView(target: self, frame: collectFrame, layout: MoreMatchWifeCell.getLayout(),
                                                  cellCls: MoreMatchWifeCell.self, id: MoreMatchWifeCell.ID)
    ...
    private func refreshData(more: Bool) {
        page = more ? page + 1 : 0
        // api
        let api = Api.request(.moreMatchWorkOurListGet(kind: MatchPeriod.MATCH_KIND_WIFE_PICTURE, page: page),
                              success: { (_, _, data) in
                                if !more {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        return MoreMatchWifeCell.getCellWithData(view: collectionView, indexPath: indexPath, dataList: workList, target: self, actionImg: #selector(targetGoImg), actionCoin: #selector(targetCoin), actionPoint: #selector(targetPoint), actionMore: #selector(targetMore))
    }
    
    @objc private func targetGoImg(sender: UIGestureRecognizer) {
        MoreMatchWifeCell.goBigImg(view: sender.view, dataList: workList)
    }
