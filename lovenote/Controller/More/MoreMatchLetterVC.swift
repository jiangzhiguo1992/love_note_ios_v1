...
class MoreMatchLetterVC: BaseVC {
    ...
    // view
    private var pagerVC: MoreMatchLetterPagerVC?
    ...
    public static func pushVC() {
        ...
        AppDelegate.runOnMainAsync {
            RootVC.get().pushNext(MoreMatchLetterVC(nibName: nil, bundle: nil))
        }
    }
    ...
    override func initView() {
        ...
        // pager
        pagerVC = MoreMatchLetterPagerVC.get(height: self.view.frame.size.height)
...
class MoreMatchLetterPagerVC: ButtonBarPagerTabStripViewController {
    ...
    public static func get(height: CGFloat) -> MoreMatchLetterPagerVC? {
        let vc = MoreMatchLetterPagerVC(nibName: nil, bundle: nil)
        // list
        let periodVC = MoreMatchPeriodListVC.get(height: height - vc.barHeight, kind: MatchPeriod.MATCH_KIND_LETTER_SHOW)
        let ourVC = MoreMatchLetterOurVC.get(height: height - vc.barHeight)
