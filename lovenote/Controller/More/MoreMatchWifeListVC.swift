...
class MoreMatchWifeListVC: BaseVC, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public static func pushVC(pid: Int64, showNew: Bool) {
        ...
        AppDelegate.runOnMainAsync {
            let vc = MoreMatchWifeListVC(nibName: nil, bundle: nil)
            vc.pid = pid
            vc.showNew = showNew
    ...
    override func initView() {
        ...
        // collect
        collectView = ViewUtils.getCollectionView(target: self, frame: self.view.bounds, layout: MoreMatchWifeCell.getLayout(),
    ...
    @objc private func targetGoAdd(sender: UIGestureRecognizer) {
        PickerHelper.selectImage(target: self, maxCount: 1, gif: true, compress: true, crop: false) { (datas) in
            ...
            OssHelper.uploadMoreMatch(data: datas[0], success: { (_, oss) in
                self.apiAdd(ossKey: oss)
            }, failure: nil)
        }
    }
    
    func apiAdd(ossKey: String) {
        let body = MatchWork()
        body.matchPeriodId = pid
        body.contentImage = ossKey
        // api
