...
class NoteAngryVC: BaseVC, UITableViewDelegate, UITableViewDataSource {
    ...
    // var
    private var angryList: [Angry]?
    private var page = 0
    private var searchIndex = 0
    
    public static func pushVC() {
        AppDelegate.runOnMainAsync {
            RootVC.get().pushNext(NoteAngryVC(nibName: nil, bundle: nil))
        }
    }
    ...
    override func initView() {
        ...
        // tableView
        tableView = ViewUtils.getTableView(target: self, frame: self.view.bounds, cellCls: NoteAngryCell.self, id: NoteAngryCell.ID)
    ...
    override func initData() {
        // notify
        NotifyHelper.addObserver(self, selector: #selector(notifyRefresh), name: NotifyHelper.TAG_ANGRY_LIST_REFRESH)
        NotifyHelper.addObserver(self, selector: #selector(notifyListItemDelete), name: NotifyHelper.TAG_ANGRY_LIST_ITEM_DELETE)
        NotifyHelper.addObserver(self, selector: #selector(notifyListItemRefresh), name: NotifyHelper.TAG_ANGRY_LIST_ITEM_REFRESH)
        // refresh
    ...
    @objc func notifyListItemDelete(notify: NSNotification) {
        let index = ListHelper.findIndexByIdInList(list: angryList, obj: notify.object)
        if index < 0 || angryList == nil || angryList!.count <= index {
            return
        }
        angryList?.remove(at: index)
        NoteAngryCell.refreshHeightMap(refresh: true, start: 0, dataList: angryList)
    ...
    @objc func notifyListItemRefresh(notify: NSNotification) {
        ...
        angryList?[index] = (notify.object as? Angry) ?? Angry()
        NoteAngryCell.refreshHeightMap(refresh: true, start: 0, dataList: angryList)
    ...
    private func refreshData(more: Bool) {
        ...
        // api
        let api = Api.request(.noteAngryListGet(who: searchType, page: page),
                              success: { (_, _, data) in
                                NoteAngryCell.refreshHeightMap(refresh: !more, start: self.angryList?.count ?? 0, dataList: data.angryList)
                                if !more {
                                    self.angryList = data.angryList
                                    self.endScrollDataRefresh(scroll: self.tableView, msg: data.show, count: data.angryList?.count ?? 0)
                                } else {
                                    self.angryList = (self.angryList ?? [Angry]()) + (data.angryList ?? [Angry]())
                                    self.endScrollDataMore(scroll: self.tableView, msg: data.show, count: data.angryList?.count ?? 0)
                                }
        }, failure: { (_, msg, _) in
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return angryList?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return NoteAngryCell.getCellHeight(view: tableView, indexPath: indexPath, dataList: angryList)
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        return NoteAngryCell.getCellWithData(view: tableView, indexPath: indexPath, dataList: angryList)
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        NoteAngryCell.goAngryDetail(view: tableView, indexPath: indexPath, dataList: angryList)
    }
