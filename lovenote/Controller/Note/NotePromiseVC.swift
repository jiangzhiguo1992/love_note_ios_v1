...
class NotePromiseVC: BaseVC, UITableViewDelegate, UITableViewDataSource {
    ...
    public static func pushVC(select: Bool = false) {
        AppDelegate.runOnMainAsync {
            let vc = NotePromiseVC(nibName: nil, bundle: nil)
            if select {
    ...
    override func initView() {
        // navigationBar
        initNavBar(title: isFromSelect() ? "please_select_promise" : "promise")
        ...
        // tableView
        tableView = ViewUtils.getTableView(target: self, frame: self.view.bounds, cellCls: NotePromiseCell.self, id: NotePromiseCell.ID)
    ...
    override func initData() {
        // notify
        NotifyHelper.addObserver(self, selector: #selector(notifyRefresh), name: NotifyHelper.TAG_PROMISE_LIST_REFRESH)
        NotifyHelper.addObserver(self, selector: #selector(notifyListItemDelete), name: NotifyHelper.TAG_PROMISE_LIST_ITEM_DELETE)
        NotifyHelper.addObserver(self, selector: #selector(notifyListItemRefresh), name: NotifyHelper.TAG_PROMISE_LIST_ITEM_REFRESH)
        // refresh
    ...
    @objc func notifyListItemDelete(notify: NSNotification) {
        let index = ListHelper.findIndexByIdInList(list: promiseList, obj: notify.object)
        if index < 0 || promiseList == nil || promiseList!.count <= index {
            return
        }
        promiseList?.remove(at: index)
        NotePromiseCell.refreshHeightMap(refresh: true, start: 0, dataList: promiseList)
    ...
    @objc func notifyListItemRefresh(notify: NSNotification) {
        ...
        promiseList?[index] = (notify.object as? Promise) ?? Promise()
        NotePromiseCell.refreshHeightMap(refresh: true, start: 0, dataList: promiseList)
    ...
    private func refreshData(more: Bool) {
        ...
        // api
        let api = Api.request(.notePromiseListGet(who: searchType, page: page),
                              success: { (_, _, data) in
                                NotePromiseCell.refreshHeightMap(refresh: !more, start: self.promiseList?.count ?? 0, dataList: data.promiseList)
                                if !more {
                                    self.promiseList = data.promiseList
                                    self.endScrollDataRefresh(scroll: self.tableView, msg: data.show, count: data.promiseList?.count ?? 0)
                                } else {
                                    self.promiseList = (self.promiseList ?? [Promise]()) + (data.promiseList ?? [Promise]())
                                    self.endScrollDataMore(scroll: self.tableView, msg: data.show, count: data.promiseList?.count ?? 0)
                                }
        }, failure: { (_, msg, _) in
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return promiseList?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return NotePromiseCell.getCellHeight(view: tableView, indexPath: indexPath, dataList: promiseList)
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        return NotePromiseCell.getCellWithData(view: tableView, indexPath: indexPath, dataList: promiseList)
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if isFromSelect() {
            NotePromiseCell.selectPromise(view: tableView, indexPath: indexPath, dataList: promiseList)
        } else {
            NotePromiseCell.goPromiseDetail(view: tableView, indexPath: indexPath, dataList: promiseList)
        }
    }
